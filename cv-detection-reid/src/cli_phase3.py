...
def cmd_reid_eval(cfg, args) -> int:
    ...
    print("\n" + render_table(
        ["video", "events found", "scorable", "recovered", "M17 rate",
         "gallery restores", "FPS"], rows_out))
...
def cmd_cross_camera(cfg, args) -> int:
    ...
    sections = [
        ...
        ("How hard this test actually is", (
            "**Read this before quoting the number.** On the bundled two-view scenes the "
            "second camera is the same scene under a horizontal pan, so the two views share "
            "lighting, scale and viewpoint. That makes the hand-off substantially easier "
            "than a real camera pair, where viewpoint, exposure and colour balance all "
            "differ — the conditions a ReID-trained backbone exists to handle and an "
            "ImageNet backbone does not." + "\n\n" +
            "A high rate here demonstrates that the **mechanism** works end to end: "
            "embeddings are built per camera, gated by class, compared by cosine distance "
            "and thresholded at the calibrated `tau_reid`. It does not establish the rate "
            "that would hold on genuinely different viewpoints. Re-run this command on a "
            "real two-camera clip before treating M18 as a deployment number.")),
    ]
